...
export const authontication = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const user = await _userRepo.findOne({ filter: { email: decoded.email } });
    ...
    if (!decoded.jti) {
        throw new AppError("invalid token jti")
    }

    const revokeToken = await redisService.getValue(redisService.revokedKey({ userId: user._id, jti: decoded.jti }))
